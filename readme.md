# Fantasy

Este repo ayuda a crear el mejor equipo y asignar roles del fantasy de hltv.  

## IEM Krakow playoffs 26

Equipo: ropz (support), zywoo(multi), magix(eco), majer(noob), woxic(awp)

| Round\Player | ropz | zywoo | magixx | majer | woxic |  |
|--------------|---------|-----------|------|------|------|--------|
| Round 1 | (avenger) | (top) | (farmer) | (assist) | (clutch) |  |
| Round 2 | (kast) | (pistol) | (rambo) | (bait) | (kobe) |  |
| Round 3 | (cannon) | (carry) | () | () | () |  |



Total Points:   
Player:   
Team:   
Rol:   
Boost:
 
## Previous results

| Event | Total Points | Points to leaderboard | Player | Team | Rol | Booster |
|-------|--------------|----|------|------|-----|---------|
|Blast Bounty 2026 - 1 | 110 (11%) | 12 | 35 (22%) | 18 (10%) | 27 (10%) | 30 (18%) |
|IEM Krakow Play In 26 | 147 (72%) | 4 | 40 (72%) | 36 (%51) | 11 (89%)  | 60 (1%) |
|IEM Krakow Stage 2 26 | 189 (%50) | 6 | 109 (24%) | 3 (84%) | 37 (66%) | 40 (32%) |
|IEM Krakow Playoffs 26 | 137 (%24) | 10 | 55 (24%) | 27 (41%) | 30 (24%) | 25 (31%) |